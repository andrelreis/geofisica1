...
def gravitational(coordinates,prisms,density,field):
    ...
    if field in ["potential", "gz","gx","gy"]:
        result *= 1e5
    return result
...
def kernel_gy(Y, X, Z):
    """
    Kernel for y component of gravitational acceleration of a prism
    """
    radius = np.sqrt(Y ** 2 + X ** 2 + Z ** 2)
    kernel = - (Z * safe_log(X + radius) +
                X * safe_log(Z + radius) -
                Y * safe_atan2(X * Z, Y * radius) )
    return kernel